...
GAME.LevelCreator2 = function() {};

GAME.LevelCreator2.prototype = {
	...
	init: function(levelId) { // add a custom variable to tell which level to load. 
		...
		//build up a grid system for displaying, store sprite information in each grid
		var w = 5, h=8;
		this.gridSystem = new Array(w);
		for (var x=0; x<w; x++){
			this.gridSystem[x] = new Array(h);
		}
		//init with null, place arrows here later
		for (var x=0; x<w; x++){
			for (var y=0; y<h; y++){
	...
	create: function() {
	    ...
	    this.levelData.arrows.forEach(function(elements){
	    	...
	    	this.gridSystem[elements.x][elements.y] = arrow;
